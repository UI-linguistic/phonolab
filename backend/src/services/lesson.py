...
def get_lesson_stats() -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: Dictionary containing lesson statistics
    """
    try:
        ...
        # Get vowels without lessons
        vowels_without_lessons = db.session.query(Vowel).outerjoin(Lesson).filter(Lesson.id == None).all()
        ...
        return {
            "error": f"Error getting lesson stats: {str(e)}"
        }
